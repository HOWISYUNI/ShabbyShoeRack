def wordPattern(pattern, strList):
    ...
    예를 들어서, aabb 와 ['elice', 'elice', 'alice', 'alice'] 가 주어졌을 경우에는 함수가 True를 반환해야 합니다. 이 경우에는 a가 elice에, b가 alice에 대응되도록 하면 배열을 해당 패턴으로 표현하는 것이 가능하기 때문이죠.
    ...
    for idx, key in enumerate(pattern):
        tempValue = patternDict.get(key, [])
        tempValue.append(idx)
        patternDict[key] = tempValue
        
        
    for candidate in patternDict.values():
        temp = []
        for idx in candidate:
            temp.append(strList[idx])
        if len(set(temp)) != 1:
            return False
    else:
        return True
