...
이때 이 연결 리스트를 순회하면서 순서를 뒤집어서, 뒤집힌 연결 리스트를 반환하는 함수를 구현 해 봅시다.
...
# head 노드가 주어졌을 때, 해당 링크드 리스트를 뒤집은 후 뒤집힌 링크드 리스트의 head 노드를 반환하는 함수를 구현 해 보세요.
def reverseLinkedList(head):
    ...
    valueList = []
    while stack:
        valueList.append(stack.pop().val)
            
    return toLinkedList(valueList).head
